...
- **Password-style masking (3.17.2)** — the token box shows dots (•••) instead of
  plaintext while you type, like a normal password field, so it can't be read over
  your shoulder or caught in a screenshot. A **Show key** toggle beneath the box
  reveals what you've currently typed if you want to double-check it before saving.
  This only ever affects text you're actively typing — an already-saved key is
  still never sent back to the menu at all (see *API token security* below). The
  same masking + Show key toggle is on the personal key box in **My Settings**
  (`/ai mymenu`).
